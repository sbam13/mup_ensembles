...
def load_checkpoint(dir, step, fname):
    ens_size = int(fname.split('_')[1])
    ckpt_dir = os.path.join(dir, fname)
...
def get_wcsl(base_dir, width_ckpt_map):
...
def get_logits_labels(dir: str, fnames: list, recorded: list):
    tls = {}
    vls = {}
    ...
    for step in recorded:
        ...
        try:
            for fname in fnames:
                sub_ens_d = load_checkpoint(dir, step, fname)
...
def main(base_dir, save_dir):
    ...
    wcsl = get_wcsl(base_dir, width_ckpt_map)
...
if __name__ == '__main__':
    # read the first two sys args as 'base_dir' and 'save_dir'
    try:
        base_dir = sys.argv[1]
        save_dir = sys.argv[2]
    except:
        print('Usage: python3 get_stats.py <base_dir> <save_dir>')
        sys.exit(1)
