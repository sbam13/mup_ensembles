...
if __name__ == '__main__':
    ...
    widths = [64]
    width_es_map =  {64: 6}
    alphas = (64.0 ** (-2), 64.0 ** (-1), 64.0 ** (-0.5), 64 ** (0.5))

    data_seed = 2423
    seed = 3442
    dp = DataParams(data_seed=data_seed)

    tlcs = [TaskListConfig(task_list=[TaskConfig(training_params=TrainingParams(), model_params=ModelParams(N=64, ensemble_size=width_es_map[64], alpha=a), seed=seed)], data_params=dp) for a in alphas]
