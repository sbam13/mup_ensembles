...
class ResNet(nn.Module):
  ...
  @nn.compact
  def __call__(self, x):
    conv = partial(self.conv, use_bias=False, dtype=self.dtype, kernel_init=self.kernel_init)
    ...
    x = self.dense(self.num_classes, dtype=self.dtype, kernel_init=self.kernel_init)(x)
    x = jnp.asarray(x, self.dtype)
    return x


MiniResNet18 = partial(ResNet, stage_sizes=(2, 2),
                   block_cls=ResNetBlock, conv=common.NTK_Conv,
                   dense = common.NTK_Dense)
