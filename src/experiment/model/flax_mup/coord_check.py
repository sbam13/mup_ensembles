...
FDICT = {
    'l1': lambda x: jnp.abs(x).mean(dtype=jnp.float32),
    'l2': lambda x: (x ** 2).mean(dtype=jnp.float32) ** 0.5,
    'mean': lambda x: x.mean(dtype=jnp.float32),
    'std': lambda x: x.std(dtype=jnp.float32),
    'covl1': lambda x: jnp.abs(cov(x)).mean(dtype=jnp.float32),
    'covl2': lambda x: (cov(x) ** 2).mean(dtype=jnp.float32) ** 0.5,
    'covoffdiagl1': lambda x: jnp.abs(covoffdiag(x)).mean(dtype=jnp.float32),
    'covoffdiagl2': lambda x: (covoffdiag(x) ** 2).mean(dtype=jnp.float32) ** 0.5
}
        ...
        if input_fdict:
            if isinstance(input, (tuple, list)):
                ...
                for name, out in input.items():
                    ...
                    get_stat(_ret, out, input_fdict)
            elif isinstance(input, jnp.ndarray):
                _ret = copy(ret)
...
def _get_coord_data(models: T.Dict[int, T.Callable], dataloader, optcls, nsteps=3,
    ...
    for i in range(nseeds):
        ...
        for width, make_model in models.items():
            model, variables, mup_state = make_model()
            model: nn.Module
...
def get_coord_data(models: T.Dict[int, T.Callable], dataloader, optimizer='sgd', lr=None, mup=True,
    ...
    if optimizer == 'sgd':
        ...
        optcls = optax.adam(lr)
    elif optimizer == 'adamw':
        optcls = optax.adamw(lr)
