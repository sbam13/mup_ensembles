...
def initialize(keys: chex.PRNGKey, init_fn, devices: list[Device]) -> FrozenDict:
    ...
    return pmap(init_fn)(keys, replicated_dummy)
...
def apply(key, data, devices, model_params, training_params):
    ...
    init_fn, apply_fn, _ = MyrtleNetwork(N, depth=5)
    # ---------------------------------------------------------------
